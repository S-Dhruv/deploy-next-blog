...
export async function getOrCreateSystemPlugin(db: DatabaseAdapter): Promise<Plugin> {
    ...
    if (!systemPlugin) {
        // Fallback: Create a dummy system plugin if installation fails/race condition
        // This prevents "Cannot read properties of null (reading '_id')"
        console.warn('System plugin not found after installation attempt. Creating fallback.');
        systemPlugin = await db.plugins.create({
            ...
            author: 'System',
            url: '', // Dummy URL for fallback
            // @ts-ignore - permissions not in PluginData but needed for logic
            permissions: [],
            hooks: {}
        });
    }
    ...
    if (!systemPlugin) {
        console.error("CRITICAL: Failed to get or create system plugin. Returning null ID.");
        // Throwing here would be caught by initializeSystem, so it's safe to throw to stop execution flow if needed,
        // but initializeSystem swallows it.
        // However, returning undefined/null might cause issues downstream.
        throw new Error("Failed to get or create system plugin");
    }
